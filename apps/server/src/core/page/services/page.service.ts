...
export class PageService {
  ...
  async create(
    ...
    // Handle markdown content if provided
    let content: any;
    let textContent: string | undefined;
    let ydoc: Buffer | undefined;

    if (createPageDto.markdownContent) {
      const html = await markdownToHtml(createPageDto.markdownContent);
      content = htmlToJson(html);
      textContent = jsonToText(content);
      ydoc = this.createYdocBuffer(content);
    }
    ...
    if (textContent) {
      insertData.textContent = textContent;
    }
    if (ydoc) {
      insertData.ydoc = ydoc;
    }

    const createdPage = await this.pageRepo.insertPage(insertData);

    return createdPage;
  }

  private createYdocBuffer(prosemirrorJson: object): Buffer {
